...
class YourMovingObject(
    ...
    override fun move() {
        currentCoordinates.x += direction.x * speed
        currentCoordinates.y += direction.y * speed
    }
...
fun main() {
    val movingObject = YourMovingObject(Vector2(0, 0), 10, Vector2(1, 1))
    movingObject.move()
    println(movingObject.currentCoordinates)
    movingObject.speedUp(10)
    movingObject.move()
    println(movingObject.currentCoordinates)
    movingObject.slowDown(5)
    movingObject.move()
    println(movingObject.currentCoordinates)
}
